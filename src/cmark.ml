...
let _ =
  let rec loop = function
    | n :: ns -> begin
        try
          ignore @@ Dl.dlopen ~filename:n ~flags:[Dl.RTLD_LAZY; Dl.RTLD_GLOBAL]
...
let node_free' =
  foreign "cmark_node_free" (ptr void @-> returning void)
...
let fopen =
  foreign "fopen" (string @-> string @-> returning (ptr void))

let fclose =
  foreign "fclose" (ptr void @-> returning int)
